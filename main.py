...
class Board:  # класс, реализующий игровое поле
    ...
    def turn(self, direction):
        ...
        if direction == 'right':
            ...
            for y in range(4):
                for x in range(4):
                    self.disg_board[y][x] = self.board[3-y][3-x]
        elif direction == "down":
            for x in range(4):
                for y in range(4):
                    self.disg_board[y][x] = self.board[x][3-y]
        else:
            for x in range(4):
                for y in range(4):
                    self.disg_board[y][x] = self.board[3-x][y]
            

    def re_turn(self, direction):
        
        if direction == 'right':
            self.board = self.disg_board
            
        elif direction == "left":
            for y in range(4):
                for x in range(4):
                    self.board[y][x] = self.disg_board[3-y][3-x]

        elif direction == "up":
            for x in range(4):
                for y in range(4):
                    self.board[y][x] = self.disg_board[x][3-y]
                    
        else:
            for x in range(4):
                for y in range(4):
                    self.board[y][x] = self.disg_board[3-x][y]
